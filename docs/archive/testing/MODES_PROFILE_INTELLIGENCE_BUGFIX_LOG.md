...
- **Commands run:**
  ```
  node --test electron/services/__tests__/Mode*.test.mjs electron/services/__tests__/ProfileIntelligenceGate.test.mjs electron/services/__tests__/AnswerCueApiE2E.test.mjs
  npm test
  ```
...
### FINDING-011 — Real AnswerCue API smoke test hits an unresolved domain with the wrong auth header

- **Mode affected:** none at runtime (test-only). Production code uses the correct endpoint.
- **Scenario / user affected:** Anyone running `RUN_ANSWERCUE_API_E2E=1 npm run test:modes` expecting the real API smoke test to validate connectivity.
- **Before behavior:** `valid auth — health endpoint responds` failed with `Expected 2xx or 404 for /v1/health; got 0 (fetch failed)`. The test hit `https://api.answercue.app/v1/health`, which is `NXDOMAIN`. It also sent `Authorization: Bearer <key>`. The earlier live-response eval succeeded because it goes through `LLMHelper` which uses the correct endpoint and header.
- **Expected behavior:** The test must hit the same base URL `LLMHelper.ts:1709` uses (`https://api.github.com/FarzamHejaziK/AnswerCue`) and send the same auth header style (`x-answercue-key: <key>`). A 404 from `/v1/health` is an accepted outcome because the route is intentionally absent — the goal is connectivity + auth, not health.
...
- **Root cause:** Drift between the test fixture defaults (`api.answercue.app`, `Authorization: Bearer`) and the production base URL/headers (`api.github.com/FarzamHejaziK/AnswerCue`, `x-answercue-key`) defined in `electron/LLMHelper.ts:1709-1720`.
- **Files changed:** `electron/services/__tests__/AnswerCueApiE2E.test.mjs` — corrected `API_BASE` default and `authHeader()` to use `x-answercue-key` for `ANSWERCUE_API_KEY`.
- **Fix applied:**
  ```diff
  - const API_BASE = process.env.ANSWERCUE_API_BASE ?? 'https://api.answercue.app';
  + const API_BASE = process.env.ANSWERCUE_API_BASE ?? 'https://api.github.com/FarzamHejaziK/AnswerCue';
  ...
  -   if (KEY) return { Authorization: `Bearer ${KEY}` };
  +   if (KEY) return { 'x-answercue-key': KEY };
  ```
- **Tests added:** none (existing 3 tests now pass against the real API).
- **Commands run:**
  ```
  ANSWERCUE_API_KEY=<redacted> RUN_ANSWERCUE_API_E2E=1 node --test electron/services/__tests__/AnswerCueApiE2E.test.mjs
  ```
- **After behavior:** 3/3 tests pass; `/v1/health` returns 404 (intentional) within ~1.7s and invalid-auth probe does not return 200.
- **Remaining risk:** If the base URL or auth header changes again, this test will drift again. Consider importing the constants from `LLMHelper` instead of duplicating them in the test, or adding a lint rule that flags `api.answercue.app` in the repo.
...
### FINDING-012 — AnswerCue API LLM emits buggy two-sum (`complement = target, num` tuple) intermittently

- **Mode affected:** technical-interview (and any custom mode that surfaces a two-sum-style problem through the AnswerCue API).
...
- **Commands run:**
  ```
  ANSWERCUE_API_KEY=<redacted> ANSWERCUE_LIVE_LLM_TESTS=1 ANSWERCUE_EVAL_SUITE=baseline npx tsx electron/test/modes-live-response-eval.ts
  ```
