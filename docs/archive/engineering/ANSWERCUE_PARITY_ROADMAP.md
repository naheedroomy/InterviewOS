# AnswerCue → legacy overlay Parity Roadmap

**Companion to:** `ANSWERCUE_PARITY_FIX_LOG.md`
...
* **3.2 UI cards (DynamicActionBar / DynamicActionCard / ActionEvidencePopover)**
  * Files: create `src/components/dynamic-actions/{DynamicActionBar,DynamicActionCard,ActionEvidencePopover}.tsx`; mount above answer area in `AnswerCueInterface.tsx`.
...
* **4.2 UI indicator**
  * Files: `src/components/AnswerCueInterface.tsx`.
...
* **Submodules `answercue-api` and `premium`:** modified on entry, untouched here.
