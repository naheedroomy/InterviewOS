# macOS Production Readiness Audit — AnswerCue

**Status:** LIVING DOCUMENT — updated after every phase.
**Started:** 2026-05-29
**Owner of last edit:** Claude Code (autopilot)
**App version:** 2.6.0
**Goal:** Make AnswerCue behave like a real, Developer ID-signed + notarized production macOS desktop app — not a hacked ad-hoc local build. Cover signing/notarization, permissions/TCC, overlay/visibility toggle reliability, audio capture stability, premium/AnswerCue-API gating, startup performance, and UX reliability.
...
### P0 — Premium & AnswerCue API (money path)
| File | Role | Why P0 |
|------|------|--------|
...
| `src/components/settings/AnswerCueApiSettings.tsx` | AnswerCue API key UI | Key entry/validation/redaction. |
| `src/components/settings/AnswerCueProSettings.tsx` | Pro plan UI | Plan separation, upgrade copy. |
| `electron/audio/AnswerCueProSTT.ts` | Pro-tier hosted STT | Gated capability; failure messaging. |
...
| `answercue-api/server.js` + `answercue-api/lib/*` | Backend license/activation | Source of truth for plan→feature mapping (read-only reference). |
...
| `src/components/ui/TopPill.tsx`, `src/components/AnswerCueInterface.tsx` | Renderer toggle UI | Renderer state vs main desync. |
...
| `electron/audio/{Deepgram,OpenAI,Soniox,Google,Rest,AnswerCuePro}*STT.ts` | STT providers start/stop |
...
| S6 | **`--deep` signing** | `codesign --force --deep` | `--deep` is deprecated by Apple and notoriously breaks nested signatures; correct approach signs inside-out (electron-builder does this answercue when given an identity). | ⚠️ |
...
### 2.2 Premium / AnswerCue API — STRUCTURE MAPPED (audit pending Phase 2)
...
- `answercue-api/` is the authoritative backend (license check, activation, webhooks). Treat as read-only reference.
...
| 2 | Premium + AnswerCue API gating | ✅ F4 fixed + tested (money path otherwise sound; F5/F7 UX follow-ups) | 2026-05-29 |
