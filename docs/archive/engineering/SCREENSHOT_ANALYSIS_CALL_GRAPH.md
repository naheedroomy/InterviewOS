...
| `electron/LLMHelper.ts:1198-2526` | Per-provider multimodal dispatch (Gemini, OpenAI, Claude, Groq, Ollama, Codex CLI, AnswerCue, custom cURL) |
...
src/hooks/useShortcuts.ts:65 ⌘+H
  → src/components/AnswerCueInterface.tsx:2575 isShortcutPressed('takeScreenshot')
    → handlers.takeScreenshot()  (AnswerCueInterface.tsx:2493)
      → window.electronAPI.takeScreenshot()       (preload.ts:397)
        → ipcMain.invoke "take-screenshot"        (ipcHandlers.ts:266)
          ...
          ← { path, preview }
        ← { path, preview }
      → setAttachedContext([...prev, {path, preview}])  (AnswerCueInterface.tsx:1001)
```
...
src/hooks/useShortcuts.ts                          ⌘+Shift+H
  → handlers.takeSelectiveScreenshot()             (AnswerCueInterface.tsx:2505)
...
global shortcut 'general:capture-and-process'      (main.ts:428)
  ...
  → AnswerCueInterface handler [implicit via attachment + manual submit flow]
    ... still uses the same Path E to actually run OCR+LLM.
```

The handler logic in `AnswerCueInterface.tsx` lines 2003 and 2037 sends the
...
```
AnswerCueInterface.tsx:1499  handleWhatToSay()
...
```
AnswerCueInterface.tsx:1644  handleCodeHint()
...
```
streamWithAnswerCue       → multipart with base64-PNG (Sharp resized to ≤768px)
  ...
  → user accepts the chip
    → window.electronAPI.acceptDynamicAction(...)
      → 'dynamic-action-accepted' IPC
        → handleWhatToSay(promptInstruction)       (AnswerCueInterface.tsx:1499)
