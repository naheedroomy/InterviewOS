...
## 3. Status
Implemented — focused verification and independent security review passed.
...
5. If any cloud scope is denied, Ollama receives the complete payload when available; otherwise the request fails closed before any cloud provider execution because caller-owned content cannot safely be separated from `message`.
...
**After:** scope derivation always unions text, image, inferred context, and explicit caller scopes. Legacy entry points accept and merge optional trailing scopes before policy routing, so denied scopes route to Ollama or fail closed before cloud dispatch.
...
- Exercise compiled `LLMHelper` dispatch for `streamChat`, `chatWithGemini`, and `streamChatWithGemini`, asserting a secret in a denied message-owned scope never reaches cloud dispatch.
- Cover both `profile_history` and `post_call_summary` denials to prove caller-owned scope does not rely on text-marker inference; prove Ollama receives the complete payload when available.
...
- Ollama remains selectable for denied scopes when available; an unavailable local fallback blocks cloud dispatch rather than risking message-owned data leakage.
...
5. Repeat with Ollama unavailable; confirm the request reports that cloud dispatch is blocked and devtools/log output contains no raw prompt body.
...
- **Opaque message payloads:** fail closed when Ollama is unavailable because block boundaries are not retained at `LLMHelper`; this prevents caller-owned content embedded in `message` from leaking to cloud.
